...
void main() {
    ...
    // Object-fit: cover - la imagen cubre sin estirarse
  ...
  private initTexture(): void {
    ...
    Promise.all(
      ...
      images.forEach((img, i) => {
        ...
        // Guardar el contexto y aplicar clip a la celda
        ctx.save();
        ctx.beginPath();
        ctx.rect(x, y, cellSize, cellSize);
        ctx.clip();

        // Calcular recorte tipo "cover"
...
const InfiniteMenu: FC<InfiniteMenuProps> = ({ items = [] }) => {
  ...
  `}
          >
            <p className="select-none relative text-[#060010] top-[1px] text-[18px]">
              &#x2197;
            </p>
          </div>
        </>
